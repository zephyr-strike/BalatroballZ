...
SMODS.Joker{ --[Super Saiyan 4] Gogeta
    key = "gogeta4",
    config = {
        extra = {
            gogetat = 0,
    ...
    calculate = function(self, card, context)
        if context.end_of_round and context.game_over == false and context.main_eval  and not context.blueprint then
            if to_big((card.ability.extra.gogetat or 0)) >= to_big(7) then
                return {
                    func = function()
                    ...
                    extra = {
                        func = function()
                            ...
                            G.E_MANAGER:add_event(Event({
                                func = function()
                                    local joker_card = SMODS.add_card({ set = 'Joker', key = 'j_dbz_gogetablue' })
                                    if joker_card then
                            ...
                            if created_joker then
                                card_eval_status_text(context.blueprint_card or card, 'extra', nil, nil, nil, {message = "Its futile!", colour = G.C.BLUE})
                            end
                            return true
                        end,
                        colour = G.C.BLUE
